...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(e => e.Job).Select(g => new
            {
                Praca = g.Key,
        ...
        {
            IEnumerable<object> result = Emps.Select(
                e => new
                {
                    e.Ename,
                    Job = (string?)e.Job,
            ...
            IEnumerable<Dept> result = Depts.GroupJoin(Emps, dept => dept.Deptno, emp => emp.Deptno, (dept, emps) => new
                {
                    dept,
                    emps
                }).Where(e => e.emps.Count() == 0 || e.emps.Count() == 5)
                .Select(e => e.dept)
                .OrderBy(e => e.Dname);
            //result =
            return result;
    ...
    public static class CustomExtensionMethods
    {
        ...
        {
            return emps.Where(emp1 => emps.Any(emps2 => emps2.Mgr?.Empno == emp1.Empno))
                .OrderBy(e => e.Ename)
